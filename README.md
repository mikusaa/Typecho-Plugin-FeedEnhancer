...
| 正文开头长度 | `300` | 截断模式下的输出总长度为 50 至 1000 个 Unicode 字符，包含 `...` |
...
`<!--more-->` 分隔符。启用后，插件从经过主题与其他插件处理的最终正文中提取首个有效
文本块，按配置长度输出纯文本，并追加带有原文绝对地址的“阅读全文”链接。该模式优先于
Typecho 的聚合全文设置和 `<!--more-->`，不读取文章的自定义摘要字段；文章 Feed 会失去
离线全文阅读能力，评论 Feed 不受影响。
